...
$$
\mathbf{t}(u) = \frac{C'(u)}{\|C'(u)\|},\qquad \mathbf{n}(u) = \mathbf{k} \times \mathbf{t}(u)
$$
...
bool ON_NurbsCurve::Offset(double offset, ON_NurbsCurve*& result)
{
  ...
  for (int i = 0; i < cntPoint - 2; i = i + 2)
  {
    ON_3dPoint curP0 = tmpPoints[i];
    ON_3dPoint curP1 = tmpPoints[i + 1];
    ...
    LineLineIntersectionType type = ON_IntersectLineLineType(curP0,
      curP1 - curP0, nextP1, nextP0 - nextP1, param0, param1, intPt);
    if (type != LineLineIntersectionType::Intersecting)
    {
      return false;
    }
    newCtrlPts.emplace_back(ON_4dPoint(intPt));
  }
  newCtrlPts.emplace_back(ON_4dPoint(tmpPoints[cntPoint - 1]));
...
bool ON_AreEqual(double a, double b, double tol)
{
  return fabs(b - a) < tol;
}
  ...
  const ON_Interval* sub_domain) const
{
  ...
  ON_Interval domain = sub_domain ? Domain().Intersection(*sub_domain) : Domain();
  if (!domain.IsIncreasing())
  return false;

  const int sampleCount = 100;
  double best_u = domain.Min();
  double best_dist2 = DBL_MAX;

  ON_3dPoint best_pt;
  for (int i = 0; i <= sampleCount; ++i)
  {
    double u = domain.ParameterAt((double)i / sampleCount);
    ON_3dPoint pt = PointAt(u);
    double dist = pt.DistanceTo(test_point);
    if (dist < best_dist2)
    {
      best_dist2 = dist;
      best_u = u;
  ...
  const double tol = ON_SQRT_EPSILON;
  double u = best_u;
  for (int i = 0; i < max_iter; ++i)
  {
    double derivs[9]; // position, 1st deriv, 2nd deriv
    if (!Evaluate(u, 2, 3, derivs))
      break;

    ON_3dPoint P(derivs);
    ON_3dVector dP(derivs + 3);
    ON_3dVector ddP(derivs + 6);

    ON_3dVector r = P - test_point;

    double r_dot_dP = ON_DotProduct(r, dP);
    double dP_dot_dP = ON_DotProduct(dP, dP);
    double r_dot_ddP = ON_DotProduct(r, ddP);

    double denom = dP_dot_dP + r_dot_ddP;
    if (fabs(denom) < tol)
      break;

    double du = -r_dot_dP / denom;
    if (fabs(du) < tol)
      break;

    u += du;
    if (u < domain.Min()) u = domain.Min();
    if (u > domain.Max()) u = domain.Max();
  }
  *t = u;
  ON_3dPoint result = PointAt(u);
  double final_dist = result.DistanceTo(test_point);
  return (final_dist <= maximum_distance || maximum_distance <= 0.0);
}

enum class LineLineIntersectionType : int
{
  Intersecting = 0,
  Parallel = 1,
  Coincident = 2,
...
LineLineIntersectionType ON_IntersectLineLineType(
  ON_3dPoint p0, 
  ON_3dVector t0, 
  ON_3dPoint p1, 
  ON_3dVector t1, 
  ...
  ON_3dPoint& i)
{
  ON_3dVector cross = ON_CrossProduct(t0, t1);
  ON_3dVector diff = p1 - p0;
  ON_3dVector coinCross = ON_CrossProduct(diff, t1);

  if (cross.IsZero())
  {
    if (coinCross.IsZero())
    {
      return LineLineIntersectionType::Coincident;
    }
    else
    {
  ...
  ON_3dVector pd1Cross = ON_CrossProduct(diff, t1);
  double pd1Dot = ON_DotProduct(pd1Cross, cross);
  s = pd1Dot / squareLength;

  ON_3dVector pd2Cross = ON_CrossProduct(diff, t0);
  double pd2Dot = ON_DotProduct(pd2Cross, cross);
  t = pd2Dot / squareLength;

  ON_3dPoint rayP0 = p0 + t0 * s;
  ON_3dPoint rayP1 = p1 + t1 * t;
  if (ON_AreEqual(rayP0.DistanceTo(rayP1), 0.0, ON_SQRT_EPSILON))
  {
    i = rayP0;
    return LineLineIntersectionType::Intersecting;
  }
  return LineLineIntersectionType::Skew;
}
```
## 7) 테스트 결과
```cpp
...
ON_SimpleArray<double> kv;
kv.Append({ 0,0,1,2,3,3 });
ON_NurbsCurve curve = ON_NurbsCurve(3, true , degree + 1, 5);
for (int i = 0; i < kv.Count(); i++)
{
  curve.SetKnot(i, kv[i]);
}
...
//     3                        2     1           1
//     4                        3     2           1
// Control Points  5 rational points
//   index               value
//   CV[ 0] [0, 0, 0, 1] = (0, 0, 0)
//   CV[ 1] [4, 4, 0, 4] = (1, 1, 0)
//   CV[ 2] [3, 2, 0, 1] = (3, 2, 0)
...
// Control Points  5 rational points
//   index               value
//   CV[ 0] [0.070710678118654752, -0.070710678118654752, 0, 1] = (0.070710678118654752, -0.070710678118654752, 0)
//   CV[ 1] [1.0671706636788625, 0.92591827525670622, 0, 1] = (1.0671706636788625, 0.92591827525670622, 0)
//   CV[ 2] [3.0707106781186546, 1.9292893218813454, 0, 1] = (3.0707106781186546, 1.9292893218813454, 0)
//   CV[ 3] [3.9236613714630888, 0.93540577585338236, 0, 1] = (3.9236613714630888, 0.93540577585338236, 0)
//   CV[ 4] [4.910557280900008, -1.0447213595499958, 0, 1] = (4.910557280900008, -1.0447213595499958, 0)
```
