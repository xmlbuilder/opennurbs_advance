...
## 🔧 LM_NewtonClosestUV 보강 버전 (Trust-Region + LM)

이 문서는 `ON_LM_NewtonClosestUV` 루틴을 **신뢰영역(Trust-Region) 기반 Levenberg–Marquardt (LM)** 방식으로 보강한 내용입니다.  
...
2. **신뢰영역 / 감소율 평가**  
   - 예측 감소량 `pred`, 실제 감소량 `ared`  
   - 비율 `ρ = ared/pred`로 스텝 품질 평가  
   - `ρ` 높으면 λ↓, 낮으면 λ↑ (댐핑 자동화)

3. **안전 장치**  
   - det≈0 → λ 확장, 미세 진동으로 탈출  
   - 도메인 길이 기반 신뢰영역 반경으로 폭주 방지

4. **정지 조건 개선**  
   - 거리뿐 아니라 **그래디언트 크기**와 **실제 감소량 정체**도 함께 고려

5. **최선 후보 보존**  
   - 반복 중 항상 최소 f를 보존하여 실패 시에도 마지막 best (u,v) 반환
...
신뢰영역 비율: seam 민감 시 0.25

analytic(sphere/cylinder) 루트는 그대로 두고, LM은 폴백/세밀 조정 담당

트림 파이프라인에는 max_snap 게이트 추가해서 원거리 데이터 차단
...
plateau·seam 인근에서 진동 방지

빠른 수렴 구간에서는 Gauss–Newton 수준 속도로 수렴

항상 최소 후보 보존 → 실패시에도 best 해 반환
