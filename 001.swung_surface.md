...
- 프로파일 곡선( xz-평면 ):  
  $$\beta(v) = (x(v),\quad 0,\quad z(v)),\quad x(v) \ge 0$$

- 트래젝터리 곡선( xy-평면 ):  
  $$\gamma(u) = (a(u),\quad b(u))$$  
  (편의상 극좌표 $$\gamma(u)=(\rho(u)\cos\theta(u),\quad \rho(u)\sin\theta(u))$$ 로도 표기 가능)

## 2) 기본 파라메트릭 방정식

$$
S(u,v) = (x(v)a(u),\quad x(v)b(u),\quad z(v))
$$

또는 극좌표 형태로

$$
S(u,v) = (\rho(u)x(v)\cos\theta(u),\quad \rho(u)x(v)\sin\theta(u),\quad z(v))
$$
